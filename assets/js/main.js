(function ($) {
	...
	$(document).ready(function () {
			...
			ticker.forEach(function (data) {
				...
				spanTag.appendChild(
					document.createTextNode(
						`${data.displayName}  ${data.value.displayValue} `
					)
				);
		...
		function getBCVRate() {
			$.ajax({
				url: serverUrl2,
				success: function (result) {
					var tasa = (document.getElementById(
						'tasaDolarBCV'
					).innerHTML = result['tasa']);
					var fecha = (document.getElementById(
						'fechaValorDolarBCV'
					).innerHTML = 'Fecha valor: ' + result['fecha']);
					var tasa = (document.getElementById('bcv2').innerHTML =
						result['tasa']);
				},
				error: function (error) {
					console.log(
						'Error loading data from BCV' + JSON.stringify(error)
					);
				},
			});
		}
	...
	jQuery(document).ready(function (e) {
		...
		function retrieve() {
			...
			let url =
				'https://api.nytimes.com/svc/search/v2/articlesearch.json?api-key=Dsak6ljecPclOkCgEeAdB1dmdr1bEqAC&q=business';
				...
				.then((data) => {
					...
					for (var i in latestNews) {
						...
						output +=
							...
							'<img class="thumb bg_img" src="' +
							web +
							media +
							'"/>' +
							'<div class="content">' +
							'<div class="post-tags"><a href="#0">' +
							latestNews[i].news_desk +
							'</a></div>' +
							'<h3 class="post-title"><a href="' +
							latestNews[i].web_url +
							'">' +
							latestNews[i].headline.main +
							'</a></h3>' +
							'<ul class="post-meta">' +
							'<li>' +
